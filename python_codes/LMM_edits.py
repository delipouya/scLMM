...
# Original MINQE(U,I) n x n formulation -> convergence
...
# Original AI (average info) REML n x n formulation -> less computation is required, simpler formulation
def reml_ai(Kernel, X, y, verbose=True, n_iter=10, starting_point=None):
    ...
    iteration_estimate = np.array(iteration_estimate)
    score_list = np.array(score_list) ## ML estimate - how good is the convergence ? - can also be used for other parameter estimation - for score test is needed
    information_list = np.array(information_list) ## estimate's standard error (SE)
